...
predictor_path = "C:\\Users\\Administrator\\Desktop\\23\\shape_predictor_68_face_landmarks.dat"

# 源程序是用sys.argv从命令行参数去获取文件夹路径，再处理文件夹里的所有图片
# 这里我直接把图片路径写在程序里了，每运行一次就只提取一张图片的关键点
faces_path = "C:\\Users\\Administrator\\Desktop\\23\\3445.jpg"
...
def get_landmarks_m(im):
    ...
    for i in range(len(dets)):
        ...
        facepoint = numpy.np.array([[p.x, p.y] for p in predictor(im, dets[i]).parts()])

        for i in range(68):
            # 标记点
            im[facepoint[i][1]][facepoint[i][0]] = [232, 28, 8]
            cv2.putText(img, str(i+1), facepoint[i][1], [facepoint[i][0]], 0.8, (0, 0, 255), 1, cv2.LINE_AA)
    return im
